...
import { joinRoom, leaveRoom, getPublicRoom, getVisibleWords } from "../game/roomService";
import { setTopicAndRule, submitWord, postQuestion, postAnswer, endTurn, forceNextTurn, handleGuessWord, handleJudgeAction, restartGame } from "../game/gameService";
import { handleChat, getRoomChatMessages } from "../game/chatService";
  ...
  try {
    ...
    switch (action.type) {
      case "set_topic_and_rule":
        ...
        result = await setTopicAndRule(roomId, action.topic, action.endCondition);
        break;
      ...
      case "submit_word":
        ...
        result = await submitWord(roomId, playerId, action.forPlayerId, action.word);
        break;
      ...
      case "chat":
        ...
        result = await handleChat(roomId, playerId, action.text);
        break;
      ...
      case "post_question":
        ...
        result = await postQuestion(roomId, playerId, action.text);
        break;
      ...
      case "post_answer":
        ...
        result = await postAnswer(roomId, playerId, action.text);
        break;

      case "end_turn":
        result = await endTurn(roomId, playerId);
        break;

      case "force_next_turn":
        result = await forceNextTurn(roomId, playerId);
        break;
      ...
      case "guess_word":
        ...
        result = await handleGuessWord(roomId, playerId, action.text);
        break;
      ...
      case "judge_action":
        ...
        result = await handleJudgeAction(roomId, playerId, action.targetPlayerId, action.action);
        break;

      case "restart_game":
        result = await restartGame(roomId, playerId);
        break;
      ...
      case "join_room":
        ...
        const joinRes = await joinRoom(roomId, playerId, action.name);
        result = { messages: joinRes.messages };
        break;

      case "leave_room":
        const leaveRes = await leaveRoom(roomId, playerId);
        if (!leaveRes.room) {
          return res.json({
            room: null,
            chatMessages: [],
